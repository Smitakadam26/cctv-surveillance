...
const createCustomIcon = (isAlerting) => {
  return L.divIcon({
    ...
    html: `<div class="w-8 h-8 rounded-full border-4 shadow-xl flex items-center justify-center transition-all duration-300 ${
      isAlerting 
...
>
          ...
          {KNOWN_LOCATIONS.map((loc) => {
            ...
            const cameraAlerts = alerts.filter(a => a.status === 'active' && a.location.toLowerCase().includes(loc.name.toLowerCase()));
